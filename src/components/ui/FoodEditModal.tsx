...
export const FoodEditModal: React.FC<FoodEditModalProps> = ({ log, onClose, onSave }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in">
      <div className="bg-zinc-900 w-full max-w-md rounded-t-[32px] sm:rounded-[32px] p-6 border-t sm:border border-zinc-800 shadow-2xl relative">
        
        <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-zinc-800 rounded-full text-zinc-400 hover:text-white">
          <X className="w-5 h-5" />
        </button>

        <h3 className="text-xl font-bold text-zinc-100 mb-6">Editar Alimento</h3>
        ...
        <button 
          onClick={handleSave}
          disabled={isSaving}
          className="mt-8 w-full py-4 bg-primary-600 text-white rounded-2xl font-bold hover:bg-primary-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
        >
